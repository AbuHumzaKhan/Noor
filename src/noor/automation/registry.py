...
def default_registry() -> TaskRegistry:
    ...
    registry.register(TaskSpec("excel.intelligence.dashboard", "Recommend a dashboard structure from the actual dataset schema.", "noor.automation.excel_intelligence:ExcelIntelligenceSkill.dashboard_spec", "excel", ("read_data", "compute_statistics")), lambda c: intelligence.dashboard_spec(c["path"], c.get("sheet_name")))
    ...
    registry.register(TaskSpec("excel.intelligence.full_analysis", "Run a complete evidence-first Excel analysis package.", "noor.automation.excel_analysis:FullExcelAnalysisSkill.run", "excel", ("read_data", "compute_statistics", "generate_formula", "generate_report")), lambda c: full_analysis.run(c["path"], c.get("sheet_name")))
